...
class EvidenceOutboxTests(TestCase):
    ...
    def test_evidence_type_values_are_permanent(self):
        self.assertEqual([choice.value for choice in EvidenceType], [1, 2, 3, 4, 5, 10])
    ...
    def test_duplicate_validates_signature_and_all_immutable_fields_before_lookup(self):
        ...
        changed_type_payload = self.valid_payload(EvidenceType.SETTLEMENT)
        changed_type_signature = self.sign_event(account, event_id, 10, changed_type_payload)
        with transaction.atomic(), self.assertRaises(EvidenceConflict):
            queue_signed_event(
                event_id, 10, changed_type_payload, zero_hash, membership, changed_type_signature
            )
    ...
    def test_settlement_schema_rejects_bad_timestamp(self):
        with self.assertRaises(ValidationError):
            _validate_payload(
                EvidenceType.SETTLEMENT,
                self.valid_payload(EvidenceType.SETTLEMENT, ack_recorded_at="not-time"),
            )
